...
public class ${modelName}Action {
    ...
    public ${modelName}Vo detail(@UrlParam(tag = "主键") Long id) {
        return ${modelVarName}Service.findById(id<#if isSplitTable>, GatewayAuthUtil.getTenantId()</#if>).toVo(${modelName}Vo.class);
    }
    ...
    public Integer save(@BodyParam(tag = "实体类vo") ${modelName}Vo entity) {
        return ${modelVarName}Service.save(entity.toModel()<#if isSplitTable>, GatewayAuthUtil.getTenantId()</#if>);
    }
    ...
    public Integer delete(@BodyParam(tag = "删除入参") BasePageExpressionRequest request) {
        return ${modelVarName}Service.deleteByIds(request.getSelectedIds()<#if isSplitTable>, GatewayAuthUtil.getTenantId()</#if>);
    }
    ...
    public BaseResponse<List<${modelName}Vo>> page(@BodyParam(tag = "通用列表请求对象") BasePageExpressionRequest request) {
        ...
        return BaseResponse.ok(${modelVarName}Service.toVos(${modelVarName}Service.find(sql<#if isSplitTable>, GatewayAuthUtil.getTenantId()</#if>)), sql.getPagination());
    }
    ...
    public String export(@BodyParam(tag = "通用列表请求对象") BasePageExpressionRequest request) throws IOException {
        BaseResponse<List<${modelName}Vo>> result = ${modelVarName}Service.findPage(request,<#if isSplitTable>GatewayAuthUtil.getTenantId(),</#if> ALLOW_FIELDS);
...
<#if geneDataType == "tree">
    ...
    public List<${modelName}Tree> syncTreeData(@BodyParam(tag = "加载树的请求参数") @NonNull BaseTreeDataDto req) {
        ...
        return ${modelVarName}Service.findChildSync(<#if isSplitTable>GatewayAuthUtil.getTenantId(), </#if>selfId, new HashSet<>(), req.getExpressions(), ALLOW_FIELDS);
    }
    ...
    public List<${modelName}Tree> asyncTreeData(@BodyParam(tag = "加载树的请求参数") @NonNull BaseTreeDataDto req) {
        ...
        return ${modelVarName}Service.findChildAsync(<#if isSplitTable>GatewayAuthUtil.getTenantId(), </#if>parentId, new HashSet<>(), req.getExpressions(), ALLOW_FIELDS);
    }
<#else>
