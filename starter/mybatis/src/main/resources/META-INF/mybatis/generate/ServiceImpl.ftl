...
<#if geneDataType == "tree">
    ...
    @Override
    public List<${modelName}Tree> findChildAsync(<#if isSplitTable == true>String tenantId, </#if>Long parentId, Set<Long> selectedIds, String[] expressions, String[] allowFields) {
        ...
        List<${modelName}> modelDataList = find(sql<#if isSplitTable == true>, tenantId</#if>);
        return tree(parentId, modelDataList, selectedIds);
    }

    @Override
    public List<${modelName}Tree> findChildSync(<#if isSplitTable == true>String tenantId, </#if>Long selfId, Set<Long> selectedIds, String[] expressions, String[] allowFields) {
        SqlWrapper<${modelName}> sql = SqlBuilder.init();
        sql.expression(expressions, allowFields);
        sql.orderByAsc(${modelName}::getSort);
        sql.queryAll();
        List<${modelName}> queryList = find(sql<#if isSplitTable == true>, tenantId</#if>);
