 ...
 * </p>
 * @author 0neBean
 * @since 24/06/12 上午 10:19
 */
@Slf4j
public class BigDecimalCalculateUtil {

    public final static BigDecimalCalculateUtil INSTANCE = new BigDecimalCalculateUtil();
    ...
    public BigDecimalCalculateUtil() {
        this.scale = 2;
        this.roundingMode = RoundingMode.HALF_UP;
    }

    public BigDecimalCalculateUtil(int scale, RoundingMode  roundingMode) {
        this.scale = scale;
            ...
            for (Field field : declaredFields) {
                ...
                if (fieldType.equals(BigDecimal.class)) {
                    if (!field.canAccess(object)) {
                        field.trySetAccessible(); // JDK 9+ 推荐方式
                    }
    ...
    private static void applyFieldsValue(Class<?> clazz, Object object, Function<BigDecimal, BigDecimal> function) {
        try {
            ...
            Field[] declaredFields = clazz.getDeclaredFields();
            for (Field field : declaredFields) {
                // 只处理 BigDecimal 类型的字段
                if (field.getType().equals(BigDecimal.class)) {
                    // 使用 trySetAccessible 代替 setAccessible(true)
                    if (!field.canAccess(object)) {
                        field.trySetAccessible(); // JDK 9+ 推荐方式
                    }
                    // 获取字段值
    ...
    public static BigDecimal plain(Object value) {
