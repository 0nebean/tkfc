...
public class WebSocketConnection extends WebSocketListener {
    ...
    private static final String PROXY_HOST = PropUtil.getInstance().getConfigWithDefaultValue("binance.api.proxy.host", StringPool.EMPTY);
    private static final String PROXY_PORT = PropUtil.getInstance().getConfigWithDefaultValue("binance.api.proxy.port", StringPool.EMPTY);
    ...
    WebSocketConnection(WebsocketRequest request, WebSocketWatchDog watchDog) {
        this(request, watchDog, false, request.requestKey);
    }

    WebSocketConnection(WebsocketRequest request, WebSocketWatchDog watchDog, boolean autoClose, String requestKey) {
        this.requestKey = requestKey;
        this.connectionId = WebSocketConnection.connectionCounter++;
        this.request = request;
        this.autoClose = autoClose;
        OkHttpClient.Builder builder = new OkHttpClient.Builder();
        if (StringUtil.isNotBlank(PROXY_HOST) && StringUtil.isNotBlank(PROXY_PORT)) {
            builder.proxy(new Proxy(Proxy.Type.SOCKS, new InetSocketAddress(PROXY_HOST, ParseUtil.toInt(PROXY_PORT))));
        }
