...
import com.binance.client.model.user.Bracket;
import com.binance.client.model.user.LeverageBracket;
...
class RestApiRequestImpl {
    ...
    RestApiRequest<ExchangeInformation> getExchangeInformation() {
        ...
        request.jsonParser = (jsonWrapper -> {
            ...
            result.setFuturesType(jsonWrapper.getString("futuresType"));

            List<AssetExchangeInformation> assetList = new LinkedList<>();
            JsonWrapperArray assetArray = jsonWrapper.getJsonArray("assets");
            assetArray.forEach((item) -> {
                AssetExchangeInformation element = new AssetExchangeInformation();
                element.setAsset(item.getString("asset"));
                element.setMarginAvailable(item.getString("marginAvailable"));
                element.setAutoAssetExchange(item.getBigDecimal("autoAssetExchange"));
    ...
    RestApiRequest<List<Order>> postBatchOrders(String batchOrders) {
        ...
        request.jsonParser = (jsonWrapper -> {
            ...
            jsonArray.forEach(obj -> {
                if (((JSONObject) obj).containsKey("code")) {
                    OrderErrorAble responseResult = new OrderErrorAble();
    ...
    RestApiRequest<Order> modifyOrder(Long orderId, String origClientOrderId, String symbol, OrderSide side, BigDecimal quantity, BigDecimal price) {
        RestApiRequest<Order> request = new RestApiRequest<>();
        UrlParamsBuilder builder = UrlParamsBuilder.build()
                .putToUrl("symbol", symbol)
                .putToUrl("side", side)
                ...
                .putToUrl("quantity", quantity.stripTrailingZeros())
                .putToUrl("price", price.stripTrailingZeros());
    ...
    RestApiRequest<DualSidePosition> getPositionSide() {
        RestApiRequest<DualSidePosition> request = new RestApiRequest<>();
        UrlParamsBuilder builder = UrlParamsBuilder.build();
        request.request = createRequestByGetWithSignature("/fapi/v1/positionSide/dual", builder);

        request.jsonParser = (jsonWrapper -> {
            DualSidePosition result = new DualSidePosition();
    ...
    RestApiRequest<List<Object>> batchCancelOrders(String symbol, String orderIdList, String origClientOrderIdList) {
        ...
        if (StringUtil.isNotBlank(orderIdList)) {
    ...
    RestApiRequest<AccountInformation> getAccountInformation() {
        ...
        request.jsonParser = (jsonWrapper -> {
            ...
            positionArray.forEach((item) -> {
                ...
                element.setPositionInitialMargin(item.getBigDecimal("positionInitialMargin"));
                element.setOpenOrderInitialMargin(item.getBigDecimal("openOrderInitialMargin"));
                element.setLeverage(item.getInteger("leverage"));
    ...
    RestApiRequest<List<MyTrade>> getAccountTrades(String symbol, Long startTime, Long endTime,
        ...
        request.jsonParser = (jsonWrapper -> {
            ...
            dataArray.forEach((item) -> {
                ...
                element.setOrderId(item.getLong("orderId"));
                element.setMarginAsset(item.getString("marginAsset"));
                element.setPrice(item.getBigDecimal("price"));
                element.setQty(item.getBigDecimal("qty"));
                element.setQuoteQty(item.getBigDecimal("quoteQty"));
...
//        request.request = createRequestByGetWithSignature("/gateway-api//v1/public/future/data/globalLongShortAccountRatio", builder);
    ...
    public RestApiRequest<LeverageBracket> getLeverageBracket(String symbol) {
        ...
        request.jsonParser = (jsonWrapper -> {
            LeverageBracket result = new LeverageBracket();
            JsonWrapper data = jsonWrapper.getJsonObject("data");
            result.setSymbol(data.getString("symbol"));
            JsonWrapperArray bracketJsonArray = data.getJsonArray("brackets");
            List<Bracket> brackets = new ArrayList<>();
            bracketJsonArray.forEach(json -> {
                ...
                bracket.setCum(json.getInteger("cum"));
    ...
    public RestApiRequest<List<LeverageBracket>> getAllLeverageBracket() {
        RestApiRequest<List<LeverageBracket>> request = new RestApiRequest<>();
        UrlParamsBuilder builder = UrlParamsBuilder.build();
        request.request = createRequestByGetWithSignature("/fapi/v1/leverageBracket", builder);
        request.jsonParser = (jsonWrapper -> {
            List<LeverageBracket> leverageBrackets = new ArrayList<>();
            JsonWrapperArray dataArray = jsonWrapper.getJsonArray("data");
            dataArray.forEach((item) -> {
                LeverageBracket result = new LeverageBracket();
                result.setSymbol(item.getString("symbol"));
                JsonWrapperArray bracketJsonArray = item.getJsonArray("brackets");
                List<Bracket> brackets = new ArrayList<>();
                bracketJsonArray.forEach(json -> {
                    Bracket bracket = new Bracket();
                    bracket.setBracket(json.getInteger("bracket"));
